...
class FiniteField:
    ...
    # TODO: use dictionaries to cache results of a+b, a*b, and a^{-1}
    def __init__(self, q):
    ...
    def add(self, a, b):
        ...
        poly_sum = div(poly_a + poly_b, self.modulus, domain = self.base_field)[1]
    ...
    def multiply(self, a, b):
        ...
        poly_a = self._int_to_poly(a)
        poly_b = self._int_to_poly(b)
        product = div(poly_a * poly_b, self.modulus, domain = self.base_field)[1]
    ...
    def inv(self, a):
        """Returns multiplicative inverse of field element a (given as an integer)"""
...
class ProjectivePlane:
    def __init__(self, q):
        ...
        self.triples = {(0, 0, 0): q * (q + 1)}
        for i in range(q):
            self.triples[(0, 1, i)] = q * q + i
            for j in range(q):
                self.triples[(1, i, j)] = i * q + j
        # TODO: finish constructing projective plane
